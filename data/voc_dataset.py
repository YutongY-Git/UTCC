...
class VOCDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128), label_subtract=1, ignore_label=255):
        ...
        self.img_ids = [i_id.strip() for i_id in open(list_path)]#xxx.txt(
    ...
    def __getitem__(self, index):
        ...
        max_h_off = img_h - self.crop_h
        h_off = random.randint(0, max(max_h_off, 0))  # 确保 max_h_off 非负
        max_w_off = img_w - self.crop_w
        w_off = random.randint(0, max(max_w_off, 0))  # Ensure max_w_off is non-negative
...
class VOCDataTestSet(data.Dataset):
    def __init__(self, root, list_path, mean=(128, 128, 128)):
        ...
        for name in self.img_ids:
            img_file = osp.join(self.root, "c-2\%s.tif" % name)
            print(img_file)
            self.files.append({
                "img": img_file
            })
...
class VOCDataTestSet_all(data.Dataset):
    def __init__(self, root, list_image_path, mean=(128, 128, 128)):
        ...
        for name in self.img_ids:
            img_file1 = osp.join(self.root, "%s" % name[0], "%s.tif" % name[1])
